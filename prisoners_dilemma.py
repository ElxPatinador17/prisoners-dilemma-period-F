...
def get_action(player, history, opponent_history, score, opponent_score, getting_team_name=False):
    ...
    #
        ...
        if getting_team_name:
            return 'backstabber vengeful'
        else:
            '''This player betrays at first, but if the partner colluded and was
            screwed, this player anticipates for them to be vengeful. He betrays 
            again, so that his partner gets dragged down when he does. The third 
            time, he colludes, because he thinks his partner will be emotionally
            damaged and will not talk'''
            ...
            if len(opponent_history)==0: #It's the first round: betray
                ...
                return 'c' # get away with nothing; they won't talk
            elif history[-1]=='c' and opponent_history[-1]=='c':
                return 'b' # they shouldn't have gotten off easy
            else:
                return 'b' #we tried in every way to survive. Don't trust anyone.
